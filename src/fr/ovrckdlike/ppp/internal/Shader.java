...
public class Shader {
	...
	private Shader(String name, int id) {
		...
		if(glGetProgrami(id, GL_VALIDATE_STATUS) == GL_FALSE)
			throw new IllegalArgumentException("Unable to validate shader program " + name + " ! " + glGetProgramInfoLog(id));
		return new Shader(name, id);
	}
	
	private static int loadShaderFile(String shaderPath, int type) throws IOException, IllegalArgumentException {
		...
		if(glGetShaderi(shader, GL_COMPILE_STATUS) == GL_FALSE)
			throw new IllegalArgumentException("Unable to parse the shader " + shaderPath + " !\n" + glGetShaderInfoLog(shader));
		return shader;
	}
