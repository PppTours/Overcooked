...
public class SkinSelect extends Scene{
	...
	public static SkinSelect get() {
		if (instance == null) instance = new SkinSelect();
		return instance;
	}
	
	private SkinSelect() {
		navigate = false;
		title = new Text("Select a character", new Rectangle(960f, 750f, 700f, 50f), Color.black);
	...
	public void run() {
		...
		if (rightp1) selectSkinP1.moveSelectionRight();
		if (leftp1) selectSkinP1.moveSelectionLeft();
		if (!rightp1 && !leftp1) selectSkinP1.resetMove();
		
		if (rightp2) selectSkinP2.moveSelectionRight();
		if (leftp2) selectSkinP2.moveSelectionLeft();
		if (!rightp2 && !leftp2) selectSkinP2.resetMove();
